...
RUN echo "deb http://mirrors.163.com/debian stretch main" > /etc/apt/sources.list \
    && apt-get update \
    && apt-get install -y transifex-client git git-review curl jq python3 && echo "deb http://packages.deepin.com/deepin unstable main" >> /etc/apt/sources.list && apt-get update && apt-get -y --allow-unauthenticated install deepin-gettext-tools && sed -i '$d' /etc/apt/sources.list && apt-get update
